...
public class Employee extends User {
	...
	private static String[] column = {"PurchaseId", "customer name","ProductId", "Cost", "Quantity", "Date"};
	private static String[] delcolumns = {"productId", "productName", "price", "quantity"};
	...
	public void createEmployee() {
		String query1 = "INSERT INTO `login` VALUES ('"+userId+"','"+password+"');";
	...
	public void updateEmployee(String name, int phone, String role, double salary) {
		String query = "UPDATE `employee` SET `employeeName`='"+name+"', `phoneNumber`='+91"+phone+"', `role`='"+role+"', `salary`="+salary+" WHERE `userId`='"+this.userId+"';";
		Connection con = null;
        Statement st = null;
		System.out.println(query);
        try {
 ...
 public static DefaultTableModel viewhistory()
 {
	DefaultTableModel model = new DefaultTableModel();
		model.setColumnIdentifiers(column);
		//String query = "SELECT purchaseInfo.purchaseId, purchaseInfo.userId, product.productName, purchaseInfo.cost, purchaseInfo.amount, purchaseInfo.date FROM purchaseInfo, product WHERE (`purchaseInfo`.`userId`='"+this.userId+"' AND `purchaseInfo`.`model_no`=`product`.`model_no`) ORDER BY `date` DESC;";     
	...
	public static DefaultTableModel viewDeletedProduct()
 {
	DefaultTableModel model = new DefaultTableModel();
		model.setColumnIdentifiers(delcolumns);
		//String query = "SELECT purchaseInfo.purchaseId, purchaseInfo.userId, product.productName, purchaseInfo.cost, purchaseInfo.amount, purchaseInfo.date FROM purchaseInfo, product WHERE (`purchaseInfo`.`userId`='"+this.userId+"' AND `purchaseInfo`.`model_no`=`product`.`model_no`) ORDER BY `date` DESC;";     
		String query="select * from product_deleted;";
        Connection con = null;
        Statement st = null;
		ResultSet rs = null;
		System.out.println(query);
